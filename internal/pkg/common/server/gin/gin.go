...
import (
	...
	"go-gin-boilerplate/internal/pkg/common/response"
...
func (g *ginHttpServer) setMiddleware() {
	...
	g.router.Use(cors.New(cors.Options{
		...
		AllowedHeaders:   []string{"Content-Type", "content-type", "Origin", "Accept", "X-App-Token", "X-Requested-With", "X-Request-ID", "Authorization", "X-Client-Id", "X-Client-Version", "X-Datasource-Id", "X-User-Data"},
	}))
